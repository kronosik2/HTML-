...
function renderModulesGrid() {
    ...
    // Получаем статус обучения (если есть currentUser)
    let trainingCompleted = false;
    let adaptationAvailable = false;
    
    if (currentUser && !isAdminMode) {
        try {
            const stats = calculateTrainingStats();
            trainingCompleted = stats.completedCount === stats.total;
            adaptationAvailable = trainingCompleted && adaptationStatus?.accepted === true;
        } catch(e) {
    ...
    grid.innerHTML = `
        ...
        <div class="module-card ${!adaptationAvailable ? 'locked' : ''}" data-module="adaptation">
